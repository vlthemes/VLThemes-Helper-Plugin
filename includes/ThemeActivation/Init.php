	...
	class VLThemesThemeActivation {
		...
		public function __construct() {
			...
			if ( ThemeActivation::CheckWPPlugin( $licenseKey, $liceEmail, $this->licenseMessage, $this->responseObj, $this->theme_file, $this->product_id, $this->product_base ) ) {
				add_action( 'vlt_toolkit_print_activation_form', array( $this, 'activated_form' ) );
				add_action( 'admin_post_' . $this->slug . '_deactivate_license', array( $this, 'action_deactivate_license' ) );
				...
				add_action( 'admin_post_' . $this->slug . '_activate_license', array( $this, 'action_activate_license' ) );
				add_action( 'vlt_toolkit_print_activation_form', array( $this, 'license_form' ) );
			}
		}
						...
						<tbody>
							...
							<tr>
								...
								if ( ! empty( $this->responseObj->expire_renew_link ) ) {
									?>
									<a target="_blank" href="<?php echo esc_url( $this->responseObj->expire_renew_link ); ?>"><?php esc_html_e( 'Renew', 'vlthemes-toolkit' ); ?></a><?php } ?></td>
							</tr>
							<tr>
								...
								if ( ! empty( $this->responseObj->support_renew_link ) ) {
									?>
									<a target="_blank" href="<?php echo esc_url( $this->responseObj->support_renew_link ); ?>"><?php esc_html_e( 'Renew', 'vlthemes-toolkit' ); ?></a><?php } ?></td>
							</tr>
							<tr>
		...
		public function license_form() {
			...
			<div class="vlt-widget">
				...
				<div class="vlt-widget__content">
						...
						<p class="mb-sm"><?php printf( esc_html__( 'To activate your copy of %s, enter your purchase code and email address to register the theme.', 'vlthemes-toolkit' ), esc_html( \VLT\Toolkit\Admin\Dashboard::instance()->theme_name ) ); ?></p>
